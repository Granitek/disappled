...
const SpeechRecognition = ({ startRecognition }) => {
    ...
    useEffect(() => {
        ...
        recognition.onresult = (event) => {
            ...
            for (let i = event.resultIndex; i < event.results.length; i++) {
                const transcriptPiece = event.results[i][0].transcript.trim().toLowerCase();

                // Wykrycie słowa kluczowego "title" lub "body"
                if (transcriptPiece.includes('title')) {
                    setCurrentField('title');
                    setTranscript(''); // Czyścimy poprzednią transkrypcję
                } else if (transcriptPiece.includes('body')) {
